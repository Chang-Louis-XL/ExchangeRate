        ...
        <div class="card-body">
            ...
            <form id="createForm" action="javascript:void(0);">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code') }}" required maxlength="3" placeholder="例如：TWD">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" required placeholder="例如：新台幣">
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    $('#createForm').submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                window.location.href = "{{ route('backend.index') }}?success=幣別及匯率新增成功！";
            },
            error: function(xhr) {
                let errors = xhr.responseJSON?.errors || {};
                
                if (xhr.responseJSON?.error) {
                    $('<div class="alert alert-danger mt-3">' + xhr.responseJSON.error + '</div>').insertBefore('#createForm');
                    return;
                }
